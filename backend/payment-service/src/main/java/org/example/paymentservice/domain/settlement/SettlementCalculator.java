...
public final class SettlementCalculator {
    ...
    public static Result calculate(Input input) {
        ...
        if (input.method() == null || input.method().rateBps() == null) throw new IllegalArgumentException(
            "UNKNOWN_METHOD"
        );
        ...
        if (
            quantity != input.refundedQuantity() || quantity > input.quantity() || cash != input.remoteCancelled()
