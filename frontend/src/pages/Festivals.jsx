...
import { FestivalGridSkeleton } from '../components/Skeleton'

const PAGE_SIZE = 8
// 백엔드가 카테고리/검색 쿼리 파라미터를 지원하지 않아, 공개된 페스티벌을 한 번에 크게 받아와 기존처럼 클라이언트에서 거른다.
const FETCH_SIZE = 100
const LIST_PARAMS = { page: 0, size: FETCH_SIZE }
...
function Festivals() {
  ...
  //홈 카드 클릭 직전 프리페치·재방문 시 캐시된 목록으로 바로 그리고, 오래됐으면 뒤에서 갱신한다.
  const { data, error, isLoading: loading } = useCachedQuery(festivalListKey(LIST_PARAMS), () =>
    fetchFestivals(LIST_PARAMS),
  )
  ...
  <main className="w-full">
    <div className="mx-auto flex max-w-[1440px] flex-col gap-4 px-6 pt-5 pb-14 sm:gap-6 sm:pt-7 sm:pb-[60px]">
      ...
      {loading && <FestivalGridSkeleton count={PAGE_SIZE} />}
      ...
      {!loading && !loadError && filtered.length > 0 && (
        <>
          <div className="grid animate-fade-in grid-cols-2 gap-5 motion-reduce:animate-none md:grid-cols-3 lg:grid-cols-4">
