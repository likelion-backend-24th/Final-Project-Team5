...
const INPUT_CLASS = 'mt-2 w-full rounded-2xl border border-gray-300 bg-white px-4 py-3'
...
export default function HelperInvite() {
  ...
  useEffect(() => {
    ...
    fetchHelperInvitation(token)
      ...
      .catch(err => {
        if (!cancelled) {
          setFatal(true)
          setError(INVITE_ERROR_MESSAGES[err.response?.data?.errorCode]
            || '초대를 확인할 수 없어요. 잠시 후 다시 열어주세요.')
        }
      })
      .finally(() => {
  ...
  async function submit(event) {
    ...
    if (password.length < 8) {
      setError('비밀번호는 최소 8자 이상 입력해주세요.')
      return
    }
    if (new TextEncoder().encode(password).length > 72) {
      setError('비밀번호는 UTF-8 기준 72바이트 이내로 입력해주세요.')
      return
    }
    if (password !== confirm) {
    ...
    try {
      ...
      if (accepted) {
        ...
        setError(INVITE_ERROR_MESSAGES[code] || '활성화하지 못했어요. 연결을 확인하고 다시 시도해주세요.')
        if (INVITE_ERROR_MESSAGES[code]) setFatal(true)
      }
    } finally {
      busy.current = false
      setSubmitting(false)
  ...
  return (
    <main className="flex min-h-screen justify-center bg-gray-50 px-5 py-12">
      <div className="h-fit w-full max-w-md rounded-3xl border border-gray-200 bg-white p-7 shadow-sm">
        ...
        {loading && (
          <p role="status" className="mt-5">초대를 확인하고 있어요…</p>
        )}
        {info && (
          <div className="mt-5 space-y-2 rounded-2xl bg-blue-50 p-4 text-sm">
            <h2 className="text-lg font-bold">{info.festivalName}</h2>
            <p>
              {formatDateTime(info.festivalStartAt)} ~ {formatDateTime(info.festivalEndAt)} (한국 시간)
            </p>
            <p>
              로그인 아이디<br />
              <strong className="break-all">{info.username}</strong>
            </p>
            <p>초대 수신: {info.maskedEmail}</p>
            <p>만료: {formatDateTime(info.expiresAt)}</p>
          </div>
        )}
        {error && (
        ...
        {fatal && (
          <div className="mt-4 text-sm">
            <p>새 초대가 필요하면 주최자에게 재발송을 요청해주세요.</p>
            <Link
          ...
          <>
            {user && (
              <p className="mt-4 text-sm text-gray-600">
                현재 {user.nickname || user.username} 계정으로 로그인되어 있어요. 활성화하면 이 도우미 계정으로 전환돼요.
              </p>
            )}
            <form
              onSubmit={submit}
            ...
            >
              ...
              <p className="text-xs text-gray-500">
                8자 이상 입력해주세요. 활성화 후에는 발급된 아이디로 로그인해요.
              </p>
              <button
                disabled={submitting || isLoading}
                className="w-full rounded-2xl bg-blue-600 py-3 font-bold text-white disabled:opacity-40"
              >
                {submitting ? '활성화 중…' : '비밀번호 설정하고 시작하기'}
              </button>
            </form>
          </>
        )}
